...
def question_preprocessing2(question):
	text= re.findall(r"[a-zA-Z]{3,}", question)
	lemmatizer = WordNetLemmatizer()
	lst_word = []
	for word in text:
	...
	question_words = pd.Series(question_words)
	vectorizer = TfidfVectorizer()
	selected_data = new_df[new_df["cluster_label"] == md.first_classifier(question)]	
	tfidf_doc = vectorizer.fit_transform(selected_data['lemma'])
	ques = vectorizer.transform(question_words)
	tfidf_df = pd.DataFrame(ques.toarray(), columns=vectorizer.get_feature_names())
	return tfidf_df
